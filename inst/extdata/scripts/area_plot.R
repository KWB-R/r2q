...
plot(x = 0, y = 0, type = "n", xlab = "", 
     ...
     ylim = c(0, site_data[["area_plan"]]$Value * 100))

axis(side = 4, 
     at = site_data[["area_plan"]]$Value * 100 * c(0, 0.25, 0.5, 0.75,1),
...
for(pol in pol_groups){
  ...
  rows <- assessment1$connectable_planning[
    assessment1$connectable_planning[,1] %in% unlist(pol),]
  ...
  v[v == Inf] <- site_data[["area_plan"]]$Value * 100
